...
type Store interface {
	...
	RecoverStaleRunningJobs(context.Context, time.Duration) (int, error)
...
func (s *PostgresStore) RecoverStaleRunningJobs(ctx context.Context, staleAfter time.Duration) (int, error) {
	if staleAfter <= 0 {
	...
	result, err := s.db.ExecContext(ctx, `
		UPDATE jobs
		SET status='queued',
			error='recovered after profiler restart',
			started_at=NULL,
			finished_at=NULL
...
func nullString(v string)any{if v==""{return nil};return v}
